...
class Measurement(Resource):
    def get(self, areaId):
        conn = db_connect.connect()
        totalArea = conn.execute("SELECT COUNT(*) FROM measurement WHERE measurement_location = %d" %int(areaId));
        totalArea = [i[0] for i in totalArea.cursor]

        valueSum  = conn.execute(" SELECT sum(value) FROM measurement WHERE measurement_location = %d" %int(areaId));
        valueSum = [i[0] for i in valueSum.cursor]

        avg = {"average_measurement": valueSum[0] / totalArea[0]}
        return  jsonify(avg)


class Totallocation(Resource):
    def get(self, areaId):
        conn = db_connect.connect()
        location = conn.execute("SELECT COUNT(location_id) FROM location WHERE location_area =  %d" %int(areaId));

        result = {"number_locations": [i[0] for i in location.cursor]}
        return  jsonify(result)
...
api.add_resource(Measurement, '/area/<areaId>/average_measurement') # Route_3
api.add_resource(Totallocation, '/area/<areaId>/number_locations') # Route_3
